...
def create_docker_container_loggers(config_yaml_read: yaml, central_log_path: str) -> list:
    ...
    for key, docker_container in config_yaml_read.get('docker_container').items():
        ...
        if (
            ...
            required_settings_keys = ['container_name', 'log_name', 'max_log_file_size', 'exclude']
            error_message = (
                'The docker container logger settings are missing required keys.\n'
                + (('-' * 150) + '\n') + (('-' * 65) + 'Additional Information' + ('-' * 63) + '\n') + (('-' * 150) + '\n')
                + 'Expected Result:\n'
                f'  - software search settings key\'s = {required_settings_keys}\n\n'
                'Returned Result:\n'
                f'  - software search settings key\'s = {docker_container_keys}\n\n'
        ...
        try:
            ...
            error_message = (
                f'The logger creation for the docker container ({container_name}) failed to create.\n\n'
                + (('-' * 150) + '\n') + (('-' * 65) + 'Additional Information' + ('-' * 63) + '\n') + (('-' * 150) + '\n')
                + f'{error}\n\n'
...
def populate_startup_variables() -> dict:
    ...
    try:
        ...
        if 'docker_container' not in returned_yaml_read_config:
            error_message = (
                'The \'docker_container\' key is missing from the YAML file.\n'
                + (('-' * 150) + '\n') + (('-' * 65) + 'Additional Information' + ('-' * 63) + '\n') + (('-' * 150) + '\n')
                + 'Suggested Resolution:\n'
                '   - Please verify you have set all required keys and try again.\n\n'
                f'Originating error on line {traceback.extract_stack()[-1].lineno} in <{__name__}>\n' +
        ...
        error_message = (
            f'An error occurred while populating the startup variables.\n\n'
            + (('-' * 150) + '\n') + (('-' * 65) + 'Additional Information' + ('-' * 63) + '\n') + (('-' * 150) + '\n')
            + f'{error}\n\n'
            f'Originating error on line {error.__traceback__.tb_lineno} in <{__name__}>\n'
            + (('-' * 150) + '\n') * 2
...
def main():
    ...
    if (
        ...
        required_settings_keys = ['email_alerts', 'alert_program_errors', 'docker_container_loggers', 'email_settings']
        error_message = (
            'The settings.log file is missing required YAML configuration keys.\n'
            + (('-' * 150) + '\n') + (('-' * 65) + 'Additional Information' + ('-' * 63) + '\n') + (('-' * 150) + '\n')
            + 'Expected Result:\n'
            f'  - settings yaml key\'s = {required_settings_keys}\n\n'
            'Returned Result:\n'
            f'  - settings yaml key\'s = {startup_variables_keys}\n\n'
            'Suggested Resolution:\n'
            '   - Please verify you have set all required keys and try again.\n\n'
            f'Originating error on line {traceback.extract_stack()[-1].lineno} in <{__name__}>\n'
            + (('-' * 150) + '\n') * 2
